...
class Orchestrator:
    ...
    def _save_llm_traces(self, question: str):
        ...
        self._save_llm_traces_pretty(question, traces)

    def _save_llm_traces_pretty(self, question: str, traces: list):
        lines = [
            "# LLM Traces (Pretty)",
            "",
            f"- question: {question}",
            f"- total_calls: {len(traces) if isinstance(traces, list) else 0}",
            "",
        ]
        ...
        if not isinstance(traces, list) or not traces:
            ...
            for idx, trace in enumerate(traces, start=1):
                item = trace if isinstance(trace, dict) else {"raw": trace}
                lines.append(f"## Call {idx}")
                lines.append(f"- timestamp: {item.get('timestamp', '')}")
                lines.append(f"- call_type: {item.get('call_type', '')}")
                ...
                for field, title in block_fields:
        ...
        path = os.path.join(self.output_dir, "llm_traces_pretty.md")
        with open(path, "w", encoding="utf-8") as file:
            file.write("\n".join(lines))
