    ...
    for fname in sorted(os.listdir(source_dir)):
        ...
        try:
            ...
            for entity_name, tm in mapping.get("table_mappings", {}).items():
                primary = tm.get("primary_table", {}) if isinstance(tm, dict) else {}
                if not isinstance(primary, dict):
                    primary = {}
                legacy_file = str(tm.get("file_name", "")).strip() if isinstance(tm, dict) else ""
                mapped_file = str(primary.get("file_name", "")).strip() or legacy_file
                if mapped_file == fname:
