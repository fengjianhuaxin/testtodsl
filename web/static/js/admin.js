...
function showImportOntologyModal() {
    showModal('导入本体Excel（重建本体+映射）', `
        <div class="form-row">
            <label>Excel 路径（可留空自动取下载目录最新“人口领域本体*.xlsx / 数据要素本体*.xlsx”）</label>
            <input class="form-input" id="m-import-excel" placeholder="例如: C:\\Users\\dahe\\Downloads\\人口领域本体 (2).xlsx">
        </div>
        <div style="font-size:12px;color:var(--text-muted);line-height:1.7">
            导入规则：按本体优先重建。<br>
            Sheet1（实体列表）：第1列实体名，第2列属性名。<br>
            Sheet2（实体关系）：第1列实体A，第2列关系，第3列实体B。<br>
...
async function loadMappingForSource() {
    try {
        ...
        const relationCard = renderTableRelationsCard(mapping, tables);
        if (!Object.keys(tm).length) {
            el.innerHTML = relationCard + '<div style="color:var(--text-muted);padding:20px">无映射</div>';
            return;
        }

        let html = '';
        for (const [entity, m] of Object.entries(tm)) {
            const primaryTable = (m.primary_table && typeof m.primary_table === 'object' && !Array.isArray(m.primary_table))
                ? m.primary_table
                : {};
            const primaryTableName = primaryTable.table_name || m.table_name || '';
            const primaryFileName = primaryTable.file_name || m.file_name || '';
            const fm = m.field_mappings || {};
            const secondaryRaw = m.secondary_tables;
            const secondaryTables = Array.isArray(secondaryRaw)
                ? secondaryRaw.filter(item => item && typeof item === 'object')
                : (secondaryRaw && typeof secondaryRaw === 'object'
                    ? Object.values(secondaryRaw).filter(item => item && typeof item === 'object')
                    : []);
            const fieldSources = m.field_sources || m.field_source_mappings || {};
            const fieldSemantics = m.field_value_semantics || m.value_semantics || {};
            const fieldSourceCount = Object.keys(fieldSources).length;
            const semanticsCount = Object.keys(fieldSemantics).length;
            const selfJoinPolicy = normalizeSelfJoinPolicy(m.self_join_policy || {});
            const selfJoinBadgeClass = selfJoinPolicy.enabled ? 'badge-green' : 'badge';
            const selfJoinBadgeText = selfJoinPolicy.enabled ? '自连接 开启' : '自连接 关闭';
            html += `<div class="card">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                    <div class="card-title" style="margin:0">${entity} → ${primaryTableName || '(未配置主表)'}
                        <span class="badge badge-blue" style="margin-left:8px">辅表 ${secondaryTables.length}</span>
                        <span class="badge badge-blue" style="margin-left:8px">字段来源 ${fieldSourceCount}</span>
                        <span class="badge badge-yellow" style="margin-left:8px">值语义字段 ${semanticsCount}</span>
                        <span class="badge ${selfJoinBadgeClass}" style="margin-left:8px">${selfJoinBadgeText}</span>
                    </div>
                    <div style="display:flex;gap:6px">
                        <button class="btn btn-primary btn-sm" onclick="llmAnalyze('${entity}','${primaryFileName}')">LLM 预分析</button>
...
function renderTableRelationsCard(mapping, tables) {
    const tableRelations = normalizeTableRelations(mapping.table_relations || []);
    const tableNameSet = new Set();
    (tables || []).forEach(t => {
        if (t && t.table_name) tableNameSet.add(String(t.table_name));
    });
    tableRelations.forEach(rel => {
        const left = String(rel.left_table || rel.from_table || '').trim();
        const right = String(rel.right_table || rel.to_table || '').trim();
        if (left) tableNameSet.add(left);
        if (right) tableNameSet.add(right);
    });
    const tableCount = tableNameSet.size;

    const rowsHtml = tableRelations.length
        ? tableRelations.map(rel => {
            const left = String(rel.left_table || rel.from_table || '').trim();
            const right = String(rel.right_table || rel.to_table || '').trim();
            const joinType = String(rel.join_type || 'left').toUpperCase();
            const pairs = Array.isArray(rel.join_pairs) ? rel.join_pairs : [];
            const pairText = pairs.map(pair => {
                if (!pair || typeof pair !== 'object') return '';
                const l = String(pair.left || pair.left_field || '').trim();
                const r = String(pair.right || pair.right_field || '').trim();
                if (!l || !r) return '';
                return `${l} = ${r}`;
            }).filter(Boolean).join(' AND ');
            return `<tr><td>${escHtml(left)}</td><td>${escHtml(right)}</td><td>${escHtml(pairText || '-')}</td><td>${escHtml(joinType)}</td></tr>`;
        }).join('')
        : '<tr><td colspan="4" style="color:var(--text-muted)">未配置表关系</td></tr>';
        ...
        <div class="card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
                <div class="card-title" style="margin:0">
                    数据层表关系
                    <span class="badge badge-blue" style="margin-left:8px">关系 ${tableRelations.length}</span>
                    <span class="badge badge-blue" style="margin-left:8px">表 ${tableCount}</span>
                </div>
                <button class="btn btn-secondary btn-sm" onclick="showEditTableRelationsModal()">编辑表关系</button>
            </div>
            <div class="table-scroll">
                <table class="data-table">
                    <thead><tr><th>左表</th><th>右表</th><th>JOIN 键</th><th>JOIN 类型</th></tr></thead>
...
async function llmAnalyze(entityName, fileName) {
    if (!fileName) {
        showToast('当前实体未配置主文件，无法做LLM预分析', 'error');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            `, async () => {
                ...
                const primaryTableName = String(fileName || '').replace(/\.xlsx$/i, '');
...
function showAddMappingModal() {
    Promise.all([API.getOntology(), API.getTables()]).then(([onto, tables]) => {
        const entities = Object.keys(onto.entities || {});
        const entityOptions = entities.map(name => `<option value="${escHtml(name)}">${escHtml(name)}</option>`).join('');
        const tableNames = (tables || []).map(t => String(t.table_name || '').trim()).filter(Boolean);
        const tableOptions = ['<option value="">(可选)</option>']
            .concat(tableNames.map(name => `<option value="${escHtml(name)}">${escHtml(name)}</option>`))
            .join('');
        const fileNames = (tables || []).map(t => String(t.file_name || '').trim()).filter(Boolean);
        const fileOptions = ['<option value="">(可选)</option>']
            .concat(fileNames.map(name => `<option value="${escHtml(name)}">${escHtml(name)}</option>`))
            .join('');

        showModal('新增映射', `
            <div class="form-row"><label>实体名</label>
                <select class="form-input form-select" id="m-mentity">${entityOptions}</select>
            </div>
            <div class="form-row"><label>锚点表(可选)</label>
                <select class="form-input form-select" id="m-mtable">${tableOptions}</select>
            </div>
            <div class="form-row"><label>锚点文件(可选)</label>
                <select class="form-input form-select" id="m-mfile">${fileOptions}</select>
            </div>
        `, async () => {
            const tableName = gv('m-mtable');
            const fileName = gv('m-mfile');
            ...
            closeModal();
            await loadMappingForSource();
            showToast('创建成功，请编辑字段来源', 'success');
        });
    }).catch(() => showToast('加载新增映射数据失败', 'error'));
}

async function showEditMappingModal(entityName) {
    try {
        const [mapping, tables, onto] = await Promise.all([
            API.getMapping(),
            API.getTables(),
            API.getOntology()
        ]);
        ...
        const primaryTable = (current.primary_table && typeof current.primary_table === 'object' && !Array.isArray(current.primary_table))
            ? current.primary_table
            : {};
        const primaryTableName = primaryTable.table_name || current.table_name || '';
        const primaryFileName = primaryTable.file_name || current.file_name || '';
        const fieldSources = current.field_sources || current.field_source_mappings || {};
        const fieldSemantics = current.field_value_semantics || current.value_semantics || {};
        const selfJoinPolicy = normalizeSelfJoinPolicy(current.self_join_policy || {});
        const legacySecondaryTables = current.secondary_tables || [];
        ...
        const fileByTable = {};
        (tables || []).forEach(item => {
            const tname = String(item.table_name || '').trim();
            if (!tname) return;
            columnsByTable[tname] = Array.isArray(item.columns)
                ? item.columns.map(col => String((col || {}).name || '').trim()).filter(Boolean)
                : [];
            const fname = String(item.file_name || '').trim();
            if (fname) fileByTable[tname] = fname;
        });

        const tableOptions = ['<option value="">(可选)</option>']
            .concat(tableNames.map(name => `<option value="${escHtml(name)}" ${name === primaryTableName ? 'selected' : ''}>${escHtml(name)}</option>`))
            .join('');
        const fileOptions = ['<option value="">(可选)</option>']
            .concat(fileNames.map(name => `<option value="${escHtml(name)}" ${name === primaryFileName ? 'selected' : ''}>${escHtml(name)}</option>`))
            .join('');
        ...
        const rows = props.map((prop, idx) => {
            ...
            let srcField = String(fieldMap[prop] || '').trim();
            const sourceItem = fieldSources[prop];
            if (sourceItem && typeof sourceItem === 'object') {
                srcTable = String(sourceItem.table_name || sourceItem.table || srcTable || '').trim();
                srcField = String(sourceItem.actual_field || sourceItem.field || srcField || '').trim();
            }
        ...
        showModal(`编辑映射: ${entityName}`, `
            <div class="form-row"><label>锚点表(可选)</label>
                <select class="form-input form-select" id="m-mtable" onchange="onPrimaryTableChange()">${tableOptions}</select>
            </div>
            <div class="form-row"><label>锚点文件(可选)</label>
                <select class="form-input form-select" id="m-mfile">${fileOptions}</select>
            </div>
            <div class="form-row">
                <label>字段来源（可视化）</label>
                <div class="table-scroll">
                    <table class="data-table">
                        <thead><tr><th>本体属性</th><th>来源表</th><th>来源字段</th></tr></thead>
                        <tbody>
                            ${rows.map(row => `
                                <tr>
                                    <td>${escHtml(row.prop)}</td>
                                    <td>
                                        <select class="form-input form-select" id="m-fsrc-table-${row.rowIndex}" onchange="onMappingFieldTableChange(${row.rowIndex})">
                                            ${['<option value="">(默认锚点表)</option>']
                                                .concat(tableNames.map(name => `<option value="${escHtml(name)}" ${name === row.srcTable ? 'selected' : ''}>${escHtml(name)}</option>`))
                                                .join('')}
                                        </select>
                                    </td>
                                    <td>
                                        <select class="form-input form-select" id="m-fsrc-field-${row.rowIndex}"></select>
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
            ...
            <div class="form-row">
                ...
                <textarea class="form-input" id="m-mselfjoin" rows="8" placeholder='{"enabled":false,"mode":"disabled","candidate_pairs":[]}' >${escHtml(JSON.stringify(selfJoinPolicy, null, 2))}</textarea>
                <div style="font-size:12px;color:var(--text-muted);margin-top:6px">
                    默认 disabled（不做自连接判定，不额外调用模型）。启用示例：{"enabled":true,"mode":"llm"} 或 {"enabled":true,"mode":"explicit_only","candidate_pairs":[{"left_field":"MOTHER_CERT_NO","right_field":"CERT_NO"}]}。
                </div>
            </div>
        `, async () => {
            ...
            const fieldMappings = {};
            const fieldSourcesPayload = {};
            mappingFieldSourceEditor.rows.forEach(row => {
                const tableValue = gv(`m-fsrc-table-${row.rowIndex}`) || tableName;
                const fieldValue = gv(`m-fsrc-field-${row.rowIndex}`);
                if (!fieldValue) return;
                if (tableValue && tableName && tableValue === tableName) {
                    fieldMappings[row.prop] = fieldValue;
                    return;
                }
                fieldSourcesPayload[row.prop] = {
                    table_name: tableValue || tableName,
                    actual_field: fieldValue
                };
            });
            ...
            await API.updateEntityMapping(entityName, {
                ...
                self_join_policy: selfJoinPolicyPayload,
                secondary_tables: legacySecondaryTables
            });
            closeModal();
...
function onPrimaryTableChange() {
    const tableName = gv('m-mtable');
    const fileSelect = document.getElementById('m-mfile');
    if (!fileSelect) return;
    const fileName = mappingFieldSourceEditor.fileByTable[tableName];
    if (fileName) fileSelect.value = fileName;
}

function onMappingFieldTableChange(rowIndex, preferredField = '') {
    const tableSelect = document.getElementById(`m-fsrc-table-${rowIndex}`);
    const fieldSelect = document.getElementById(`m-fsrc-field-${rowIndex}`);
    if (!tableSelect || !fieldSelect) return;

    const selectedTable = tableSelect.value || gv('m-mtable');
    const currentValue = preferredField || fieldSelect.value || '';
    const columns = mappingFieldSourceEditor.columnsByTable[selectedTable] || [];

    const options = ['<option value="">(未映射)</option>']
        .concat(columns.map(col => `<option value="${escHtml(col)}">${escHtml(col)}</option>`));
    fieldSelect.innerHTML = options.join('');

    if (currentValue) {
        const hasValue = columns.includes(currentValue);
        if (!hasValue) {
...
async function showEditTableRelationsModal() {
    try {
        const [mapping, tables] = await Promise.all([API.getMapping(), API.getTables()]);
        const tableNames = (tables || []).map(t => String(t.table_name || '').trim()).filter(Boolean);
        const relations = normalizeTableRelations(mapping.table_relations || []);

        tableRelationEditor = { tableNames, rowCursor: 0 };
        showModal('编辑数据层表关系', `
            <div style="margin-bottom:8px;color:var(--text-muted);font-size:12px">
                这里配置的是“数据层表关系”，供SQL规划自动找JOIN路径使用。
            </div>
            <div id="m-table-relations-rows"></div>
            <div style="margin-top:10px">
                <button class="btn btn-secondary btn-sm" onclick="addTableRelationRow()">＋ 添加关系</button>
            </div>
        `, async () => {
            const container = document.getElementById('m-table-relations-rows');
            const rows = Array.from(container.querySelectorAll('.table-rel-row'));
            const payload = [];
            rows.forEach(row => {
                const rowId = row.dataset.rowid;
                const leftTable = gv(`m-tr-left-table-${rowId}`);
                const rightTable = gv(`m-tr-right-table-${rowId}`);
                const leftField = gv(`m-tr-left-field-${rowId}`);
                const rightField = gv(`m-tr-right-field-${rowId}`);
                const joinType = gv(`m-tr-join-type-${rowId}`) || 'left';
                if (!leftTable || !rightTable || !leftField || !rightField) return;
                payload.push({
                    left_table: leftTable,
                    right_table: rightTable,
                    join_pairs: [{ left: leftField, right: rightField }],
                    join_type: joinType
                });
            });
            await API.saveTableRelations({ table_relations: payload });
            closeModal();
            await loadMappingForSource();
            showToast('表关系已更新', 'success');
        });

        if (!relations.length) {
            addTableRelationRow();
        } else {
            relations.forEach(rel => addTableRelationRow(rel));
        }
    } catch (e) {
        showToast(e.message || '加载表关系失败', 'error');
    }
}

function addTableRelationRow(relation = null) {
    const container = document.getElementById('m-table-relations-rows');
    if (!container) return;
    const rowId = ++tableRelationEditor.rowCursor;
    const rel = relation && typeof relation === 'object' ? relation : {};
    const pairs = Array.isArray(rel.join_pairs) ? rel.join_pairs : [];
    const firstPair = pairs.find(item => item && typeof item === 'object') || {};
    const leftTable = String(rel.left_table || rel.from_table || '').trim();
    const rightTable = String(rel.right_table || rel.to_table || '').trim();
    const leftField = String(firstPair.left || firstPair.left_field || rel.left_field || '').trim();
    const rightField = String(firstPair.right || firstPair.right_field || rel.right_field || '').trim();
    const joinType = String(rel.join_type || 'left').trim().toLowerCase();
    const tableOptions = ['<option value="">(选择表)</option>']
        .concat(tableRelationEditor.tableNames.map(name => `<option value="${escHtml(name)}">${escHtml(name)}</option>`))
        .join('');

    const html = `
        <div class="table-rel-row" data-rowid="${rowId}" style="padding:10px;border:1px solid var(--border);border-radius:8px;margin-bottom:8px">
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 120px 56px;gap:8px;align-items:end">
                <div><label style="font-size:12px;color:var(--text-muted)">左表</label><select class="form-input form-select" id="m-tr-left-table-${rowId}">${tableOptions}</select></div>
                <div><label style="font-size:12px;color:var(--text-muted)">左字段</label><input class="form-input" id="m-tr-left-field-${rowId}" placeholder="如 cert_no"></div>
                <div><label style="font-size:12px;color:var(--text-muted)">右表</label><select class="form-input form-select" id="m-tr-right-table-${rowId}">${tableOptions}</select></div>
                <div><label style="font-size:12px;color:var(--text-muted)">右字段</label><input class="form-input" id="m-tr-right-field-${rowId}" placeholder="如 cert_no"></div>
                <div><label style="font-size:12px;color:var(--text-muted)">JOIN类型</label><select class="form-input form-select" id="m-tr-join-type-${rowId}"><option value="left">LEFT</option><option value="inner">INNER</option></select></div>
                <button class="btn btn-danger btn-sm" onclick="removeTableRelationRow(${rowId})">删</button>
            </div>
        </div>`;
    container.insertAdjacentHTML('beforeend', html);

    if (leftTable) document.getElementById(`m-tr-left-table-${rowId}`).value = leftTable;
    if (rightTable) document.getElementById(`m-tr-right-table-${rowId}`).value = rightTable;
    if (leftField) document.getElementById(`m-tr-left-field-${rowId}`).value = leftField;
    if (rightField) document.getElementById(`m-tr-right-field-${rowId}`).value = rightField;
    document.getElementById(`m-tr-join-type-${rowId}`).value = joinType === 'inner' ? 'inner' : 'left';
}

function removeTableRelationRow(rowId) {
    const row = document.querySelector(`.table-rel-row[data-rowid="${rowId}"]`);
    if (row) row.remove();
}
...
function parseJsonValueWithLabel(text, label) {
...
function normalizeSelfJoinPolicy(raw) {
    if (typeof raw === 'boolean') {
        return { enabled: raw, mode: raw ? 'llm' : 'disabled', candidate_pairs: [] };
    }
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
        return { enabled: false, mode: 'disabled', candidate_pairs: [] };
    }
    const enabled = !!raw.enabled;
    const modeRaw = String(raw.mode || (enabled ? 'llm' : 'disabled')).toLowerCase();
    const mode = ['disabled', 'llm', 'explicit_only'].includes(modeRaw)
        ? modeRaw
        : (enabled ? 'llm' : 'disabled');
    const candidatePairs = Array.isArray(raw.candidate_pairs) ? raw.candidate_pairs : [];
    return {
        enabled,
