...
2) Legacy two-sheet mode:
   sheet1(entity/property[, optional property_en]), sheet2(entityA/relation/entityB),
   then tries to adapt to existing source tables.
"""
...
def _infer_type(field_name: str) -> str:
    ...
    if any(token in str(field_name or "") for token in ("日期", "时间", "年月", "时点")):
        return "date"
    if any(token in str(field_name or "") for token in ("数量", "金额", "费用", "比例", "率", "人数", "次数", "总数", "均值")):
        return "number"
...
def _load_entity_rows(entity_sheet_df: pd.DataFrame) -> OrderedDict[str, OrderedDict[str, dict[str, Any]]]:
    ...
    if frame.shape[1] < 2:
        raise ValueError("Legacy sheet1 requires at least 2 columns: entity + property.")

    normalized_headers = {_normalize_header(col): col for col in frame.columns}

    def _pick_column(candidates: tuple[str, ...], fallback_index: int | None = None) -> Any:
        for candidate in candidates:
            actual = normalized_headers.get(_normalize_header(candidate))
            if actual is not None:
                return actual
        if fallback_index is not None and frame.shape[1] > fallback_index:
            return frame.columns[fallback_index]
        return None

    entity_col = _pick_column(("实体", "实体名", "对象", "entity", "entity_name"), 0)
    property_label_col = _pick_column(("属性", "属性名", "属性名称", "property", "property_name"), 1)
    property_key_col = _pick_column(("属性英文名", "英文属性名", "property_en", "field_en"), 2)

    selected_columns = [entity_col, property_label_col]
    if property_key_col is not None and property_key_col not in selected_columns:
        selected_columns.append(property_key_col)

    frame = frame.loc[:, selected_columns].copy().rename(
        columns={
            entity_col: "entity_cn",
            property_label_col: "property_label",
            property_key_col: "property_key_raw",
        }
    )
    if "property_key_raw" not in frame.columns:
        frame["property_key_raw"] = ""

    frame["entity_cn"] = frame["entity_cn"].ffill()

    def _normalize_property_key(raw_value: str, label_value: str) -> str:
        raw_text = _clean_text(raw_value)
        if raw_text and re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", raw_text):
            return raw_text.upper()

        label_text = _clean_text(label_value)
        if label_text and re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", label_text):
    ...
    for _, row in frame.iterrows():
        ...
        property_label = _clean_text(row.get("property_label"))
        property_name = _normalize_property_key(
            raw_value=_clean_text(row.get("property_key_raw")),
            label_value=property_label,
        )

        if not entity_cn or not property_name:
            continue

        if entity_cn not in entities:
            entities[entity_cn] = OrderedDict()

        if property_name not in entities[entity_cn]:
            entities[entity_cn][property_name] = {
                "label": property_label or property_name,
                "type": _infer_type(property_label or property_name),
            }
        elif property_label and not entities[entity_cn][property_name].get("label"):
            entities[entity_cn][property_name]["label"] = property_label
...
def _load_relation_rows(relation_sheet_df: pd.DataFrame) -> list[dict[str, str]]:
    ...
    from_col = _pick_column(("实体A", "实体a", "from_entity", "from", "source", "实体1"), 0)
    rel_col = _pick_column(("关系", "关系名", "relation", "rel", "type"), 1)
    to_col = _pick_column(("实体B", "实体b", "to_entity", "to", "target", "实体2"), 2)
    ...
    for _, row in frame.iterrows():
        ...
        rel_label = _clean_text(row.get(rel_col)) if rel_col is not None else ""
