...
3) 若无法判断，target_entities 输出空数组。"""
    ...
    def _normalize_intent_result(
        ...
        if not target_entities_raw:
            # Backward compatibility: some models still output {"entity": [...]}.
            target_entities_raw = parsed.get("entity", [])
        ...
        calc_params_raw = parsed.get("calc_params", {})
        if not isinstance(calc_params_raw, dict):
            calc_params_raw = {}

        legacy_calc_type = str(
            calc_params_raw.get("calc_type", "") or calc_params_raw.get("type", "")
        ).strip().lower()
        if legacy_calc_type and isinstance(calc_params_raw, dict):
            # Keep calc type at top-level and avoid duplicated semantics in params.
            calc_params_raw = dict(calc_params_raw)
            calc_params_raw.pop("calc_type", None)
            calc_params_raw.pop("type", None)

        calc_type = str(parsed.get("calc_type", "")).strip().lower() or legacy_calc_type or "detail"
        ...
        calc_params = self._normalize_calc_params(calc_params_raw)
