...
class CalcMethodAgent(BaseAgent):
    ...
    def _infer_calc_type(self, question: str, output_fields: list) -> tuple[str, str]:
        ...
        has_aggregate_scope = has_any(aggregate_scope_keywords)
        for field in output_fields:
            field_name = str(field.get("field", "")).upper()
            if self._has_aggregate_field_hint(field_name):
                if not has_aggregate_scope:
                    continue
                return "sum", f"\u8f93\u51fa\u5b57\u6bb5 {field_name} \u547d\u4e2d\u805a\u5408\u5b57\u6bb5\u515c\u5e95"
            if self._has_usage_field_hint(field_name):
                if not has_aggregate_scope:
                    continue
    ...
    def _pick_numeric_target_fields(self, extracted_fields: list, output_fields: list, dimensions: list | None = None) -> list:
        ...
        for item in candidates:
            ...
            field_tokens = self._split_field_tokens(field_upper)
            if field_type == "number":
                if field_upper not in seen:
                    seen.add(field_upper)
                    result.append(field_name)
                continue
            if self._has_aggregate_field_hint(field_upper, field_tokens=field_tokens) or self._has_usage_field_hint(
                field_upper,
    ...
    @staticmethod
    def _split_field_tokens(field_name: str) -> list:
        return [token for token in re.split(r"[^A-Z0-9]+", str(field_name or "").upper()) if token]

    @classmethod
    def _has_aggregate_field_hint(cls, field_name: str, field_tokens: list | None = None) -> bool:
        tokens = field_tokens if isinstance(field_tokens, list) else cls._split_field_tokens(field_name)
        token_set = set(tokens)
        return bool(token_set & {"COST", "AMOUNT", "FEE", "BUDGET", "EXPENSE", "TOTAL", "COUNT", "NUM"})

    @classmethod
    def _has_usage_field_hint(cls, field_name: str, field_tokens: list | None = None) -> bool:
        tokens = field_tokens if isinstance(field_tokens, list) else cls._split_field_tokens(field_name)
        token_set = set(tokens)
        if token_set & {"USE", "USAGE"}:
            return True
        if "CPU" in token_set and "USE" in token_set:
            return True
        if "MEM" in token_set and "USE" in token_set:
            return True
        if "DISK" in token_set and "USE" in token_set:
            return True
        return False
