...
class QualityCheckAgent(BaseAgent):
    ...
    def run(self, input_data: dict) -> dict:
        ...
        if compute_result:
            expected_fields = self._build_expected_fields(calc_rule, extracted_fields, query_spec=query_spec)
        ...
        passed = len(issues) == 0
        self.log(f"质检{'通过 ✅' if passed else '发现问题 ⚠️'}: {issues if issues else '无问题'}")
    ...
    @staticmethod
    def _build_expected_fields(calc_rule: dict, extracted_fields: list, query_spec: dict | None = None) -> set:
        field_display_map = {}
        ...
        if isinstance(query_spec, dict) and str(query_spec.get("query_mode", "")).strip().lower() == "aggregate":
            ...
            if isinstance(measures, list):
                for measure in measures:
                    ...
                    alias = str(measure.get("alias", "")).strip() or alias_defaults.get(agg, "")
                    measure_field = str(measure.get("field", "")).strip()
                    measure_field_leaf = measure_field.rsplit(".", 1)[1].strip() if "." in measure_field else measure_field
                    if (
        ...
        agg_alias = {
            ...
            "rate": {str(params.get("metric_alias", "rate_value")).strip() or "rate_value"},
        }
